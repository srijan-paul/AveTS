...
c -= 3`).toHaveTypeError("Cannot use operator '-=' on operand types 'num' and 'str'");
...
if a >= 1 #expect type error
    a := 1
`).toHaveTypeError("Cannot use operator '>=' on operands of type 'str' and 'num'.");
...
foo(1, false)`
		).toHaveTypeError("cannot assign argument of type 'bool' to parameter of type 'num'.");
